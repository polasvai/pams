...
public sealed class AuctionService(ApplicationDbContext db)
{
    ...
    public async Task<AuctionServiceResult> StartAsync(int id, CancellationToken ct = default)
    {
        ...
        if (auction.Status != AuctionStatus.Draft)
            return Fail("Only a draft auction can be started.");
        if (auction.AuctionPlayers.Count == 0)
            return Fail("Add at least one player before starting.");
    ...
    public async Task<AuctionServiceResult> SetStatusAsync(int id, AuctionStatus status, CancellationToken ct = default)
    {
        ...
        if (auction is null)
            return Fail("Auction was not found.");
        if (status == AuctionStatus.Paused && auction.Status != AuctionStatus.Live)
            return Fail("Only a live auction can be paused.");
        if (status == AuctionStatus.Live && auction.Status != AuctionStatus.Paused)
            return Fail("Only a paused auction can resume.");
        auction.Status = status;
        if (status == AuctionStatus.Completed)
            auction.EndsAt = DateTimeOffset.UtcNow;
        await db.SaveChangesAsync(ct);
    ...
    public async Task<AuctionServiceResult> PlaceBidAsync(int id, int lotId, int teamId, decimal amount, string? userId = null, CancellationToken ct = default)
    {
        ...
        if (auction.Status != AuctionStatus.Live || auction.CurrentAuctionPlayerId != lotId)
            return Fail("This player is not open for bidding.");
        ...
        if (lot.Status != AuctionPlayerStatus.OnAuction)
            return Fail("This lot is not open for bidding.");
    ...
    public async Task<AuctionServiceResult> FinishPlayerAsync(int id, bool sold, CancellationToken ct = default)
    {
        ...
        if (auction is null || auction.CurrentAuctionPlayer is null)
            return Fail("No player is on the block.");
